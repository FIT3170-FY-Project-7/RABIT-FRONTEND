    ...
    return (
        <div>
            <div style={{ display: 'flex' }}>
                <div>
                    Inactive Parameters
                    <Reorder reorderId="inactive-items" reorderGroup="param-selector" onReorder={onReorder} style={style}>
                        {items["inactive-items"].map((item, i) => (
                            <div onDoubleClick={onClick("inactive-items", i)} onClick={onClick("inactive-items", i)} style={itemStyle} key={item}>
                                {item}
                            </div>
                        ))}
                    </Reorder>
                ...
                <div>   
                    ...
                    <Reorder reorderId="active-items" reorderGroup="param-selector" onReorder={onReorder} style={style}>
                        {items["active-items"].map((item, i) => (
                            <div onDoubleClick={onClick("active-items", i)} onClick={onClick("active-items", i)} style={itemStyle} key={item}>
                                {item}
                            </div>
                        ))}
                    </Reorder>
            ...
            <div>
                ...
                <Switch checked={autoUpdate} onChange={onAutoUpdate} />Auto Update
            </div>
        </div>
    );
}
