...
const DEV_PLUGINS    = [    // Development-only plugins.
...
module.exports = {
    ...
    plugins: [
        ...
        new CopyWebpackPlugin({ patterns: [{ from: 'public', to: 'build', globOptions: { ignore: ['index.html'] } }] }),
        new BannerPlugin(`${PACKAGE.name} | ${PACKAGE.version} | ${PACKAGE.author}`),
        new ProgressPlugin(
            (percent, msg, ...args) => console.log(`${(percent * 100).toFixed(2).padEnd(7)}% | ${msg} | ${args.join(' | ')}`)
        ),
        new HtmlWebPackPlugin({
            ...
            metadata   : {                      // Metadata to be passed to the template's placeholder strings.
                title       : 'RABIT'                   ,
